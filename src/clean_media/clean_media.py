...
def rename_forced_subs(content_dir: Path):
    ...
    for lang_str in lang_strs:
        ...
        for orig_subtitle_file in content_dir.glob(f"**/*.1.{lang_str}.srt"):
            ...
            # orig_subtitle_file.rename(orig_subtitle_file_str.replace(f".1.{lang_str}.srt", ".eng.srt"))
...
def main(content_dir: Path, run_en_to_eng_rename: bool = False) -> None:
    """Runs the media cleaning for a given directory containing content.

    Args:
        content_dir (Path): The directory to run the renaming on.
        run_en_to_eng_rename (bool, optional): Whether to rename en subtitle files to eng. Defaults to False.
    """
    # subtitles
    rename_forced_subs(content_dir)
    if run_en_to_eng_rename:
...
if __name__ == "__main__":
    main(Path("/volume2/data/media/movies"), run_en_to_eng_rename=True)
